#!/bin/bash
#multiple users accessing same api
# PURPOSE :multiple users accessing same api
# INPUT:
#       1. user with userID 'ashwini' calls API 'documents' 10 times
#       2. user with userID 'manoj' calls API 'documents' 10 times
#OUTPUT:
...
curl -b 'userID=manoj' "http://localhost:8090/documents" #should run successfully max 3 times within 60s
done